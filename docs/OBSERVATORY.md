...
### 2026-09-14 (4차, slack-bot 세션) — 3차 결과를 관측소에 실제로 배선

- **heyreci 3번 칸이 채워졌다.** 3차에서 GSC 속성 권한을 줬지만 `observatory/sites.json` 은
  아직 `gsc: null` 이었다 — **권한만 주고 표는 그대로 `—` 였을 뻔했다**(3차가 `gsc.mjs` 의
  `SITES` 에서 겪은 것과 정확히 같은 자리다. 배선은 **세는 쪽마다** 따로 있다).
  `sc-domain:heyreci.com` 을 넣고 그 사이트만 `--force` 재수집 → **노출 10 · 평균순위 5.5** 실측.
- 🔴 **heyreci GSC 속성엔 사이트맵이 한 번도 제출된 적이 없다.** 라이브는 112장인데 GSC 는 0건이다.
  (필터 때문이 아니다 — 필터 없이 조회해도 `[]`.) `gsc.mjs --submit` 한 줄이면 된다.
- ✅ **"200 = 파일 있음" 오판을 고쳤다**(3차가 잡아준 함정). `seo_check.check_site_assets` 가
  상태코드로 존재를 판정하고 있었다. bb 는 **없는 경로 전부에 200 + SPA HTML** 을 준다
  (`/BingSiteAuth.xml`·`/definitely-not-here.txt` 도 200) → 실제로 **없는 `bb/llms.txt` 가
  ✅ 로 찍히고 있었다.** 이제 본문이 HTML 이면 "파일 없음"으로 적는다.
  순수 형식 판정(문서 첫머리)이라 룰 #1 대상이 아니고, 걸려도 경고 한 줄이라 아무것도 안 막는다.
  회귀 확인: notes·cue·the-moment.us·heyreci 의 진짜 robots/sitemap/llms 는 그대로 ✅.
  ⚠️ `~/bin/seo_check.py` 사본도 같이 맞췄다.

## 2026-09-14 (4차) — 사이트맵 제출 누락 **3건**. 같은 구멍이 세 군데 더 있었다

Agent-2 가 heyreci 를 배선하다 발견: **`sc-domain:heyreci.com` 에 사이트맵이 한 번도 제출된 적이 없었다**
(라이브 112장, GSC 0건). 그 김에 전 속성을 훑었더니 둘이 더 나왔다.

| 사이트 | 라이브 | GSC 제출 | 조치 |
|---|---:|---|---|
| heyreci.com | 112장 | **없음** | ✅ 제출 |
| kontext.the-moment.us | 8장 | **없음** | ✅ 제출 |
| bb.the-moment.us | 1장 | **없음** | ✅ 제출 |
| the-moment.us · cue · mark · notes · teamai | — | 있음 | — |

🔴 **이번 건의 교훈은 "권한·등록과 배선은 다른 일"이다.** 오늘 하루에 같은 모양이 **세 번** 나왔다:
1. heyreci GSC 속성에 서비스계정을 **추가했는데** `gsc.mjs` 의 `SITES` 에 없어서 안 읽혔다
2. 같은 사이트가 `observatory/sites.json` 의 `gsc: null` 이라 표에 `—` 로 남았다 (Agent-2 발견)
3. 사이트맵이 **라이브에 있는데** GSC 에 제출이 안 돼 있었다

셋 다 "한쪽만 해두고 다 된 줄 안" 것이다. **세는 쪽이 여럿이면 배선도 그 수만큼 있다.**

## 200 함정이 실제로 오판 중이었다 (Agent-2 실측)

`seo_check.check_site_assets` 가 `code == 200` 으로 파일 존재를 판정했다. bb 는 없는 경로에
SPA 랜딩을 돌려주므로 **없는 `bb/llms.txt` 가 계속 ✅ 로 찍히고 있었다.**
이제 본문이 HTML 로 시작하면 "없음"으로 적는다. 🚫 존재 여부를 상태코드로 판정하지 마라.
