...
**히어로 CTA 는 본문 CTA 보다 크다.** `--mmt-ctrl-pad-lg`(17·40) vs `--mmt-ctrl-pad`(14·28).
사장님 확정 2026-08-07: *"히어로만 크게 두는 게 맞아 그대로 둬"*.
notes `/business` 의 「기업 문의하기」가 이 경우다.

🚫 "값이 안 맞는다"고 하나로 합치지 마라. **크기 계층은 두되 값(반경·굵기·글자크기)만 공유**하는 것이
이 토큰의 설계다. 계층까지 없애면 화면에서 무엇이 가장 중요한 행동인지 안 보인다.
