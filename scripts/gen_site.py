...
PRIVACY = f"""{LEGAL_CSS_WRAP}
  ...
  <div class="lg-body">
    ...
    <ul>
      ...
      <li><b>her(음성 입력)</b>: 크롬 브라우저에 내장된 음성 인식 기능을 사용합니다. 모멘터스는 별도의 음성 인식 서버를 두지 않으며 녹음을 수집·저장하지 않습니다. 인식 처리 방식은 이용자가 사용하는 브라우저의 정책을 따릅니다.</li>
    ...
    <h3>1.5 크롬 확장 — 인스타 대신 올려주기 · heyreci</h3>
    <p>「인스타 대신 올려주기 · heyreci」는 이용자가 헤이레시에서 만든 이미지와 문구를 <b>인스타그램 입력칸에 대신 채워 넣는</b> 크롬 확장입니다. <b>개인정보를 수집·저장·전송하지 않습니다.</b></p>
    <ul>
      <li><b>계정 정보를 받지 않습니다.</b> 인스타그램 아이디·비밀번호를 요구하거나 저장하지 않으며, 이용자가 <b>이미 로그인해 둔 브라우저 안에서만</b> 동작합니다.</li>
      <li>다루는 내용은 <b>이용자가 직접 올리려고 만든 이미지·문구뿐</b>이며, 모멘터스 서버로 전송하지 않습니다.</li>
      <li>진행 상태(어느 단계까지 갔는지)는 브라우저 로컬 저장소(<code>chrome.storage</code>)에만 임시로 두며 작업이 끝나면 지웁니다.</li>
      <li>접근 권한은 <code>instagram.com</code> <b>한 곳</b>으로 한정됩니다. 다른 사이트의 내용을 읽거나 조작하지 않습니다.</li>
      <li><b>마지막 게시·저장 버튼은 이용자가 직접 누릅니다.</b> 확장은 채우고 가리키는 데서 멈춥니다.</li>
      <li>화면을 촬영하거나, 열람 기록을 수집하거나, 광고 식별자를 사용하지 않습니다.</li>
    </ul>
